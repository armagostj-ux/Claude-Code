...
const DEFAULT_BALANCE = Math.round(projectGrowth(DEFAULT_INPUTS, 18)[18].trumpAccount / 1000) * 1000

const TAX_BRACKETS = [
  { key: 'low', label: 'Low (12%)', value: 0.12 },
  { key: 'mid', label: 'Middle (22%)', value: 0.22 },
  { key: 'high', label: 'High (32%)', value: 0.32 },
] as const
type BracketKey = (typeof TAX_BRACKETS)[number]['key']
const bracketRate = (key: BracketKey) => TAX_BRACKETS.find((b) => b.key === key)!.value
...
function BracketPicker({ label, value, onChange }: { label: string; value: BracketKey; onChange: (k: BracketKey) => void }) {
  return (
    <div>
      <p className="mb-2 text-sm font-medium text-text-primary">{label}</p>
      <PillGroup options={TAX_BRACKETS.map((b) => ({ key: b.key, label: b.label }))} value={value} onChange={onChange} />
    </div>
  )
}
...
function WithdrawPanel({ balance, amount, onAmount, bracket, onBracket }: {
  balance: number
  amount: number
  onAmount: (v: number) => void
  bracket: BracketKey
  onBracket: (k: BracketKey) => void
}) {
  const [reasonKey, setReasonKey] = useState<ReasonKey>('home')
  const reason = REASONS.find((r) => r.key === reasonKey)!
  const ordinaryRate = bracketRate(bracket)
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[280px_1fr]">
      <div className="space-y-6 rounded-3xl bg-surface-card p-6">
        ...
        />
        <BracketPicker label="Your tax bracket" value={bracket} onChange={onBracket} />
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-3xl bg-surface-card p-6">
          <p className="text-xs font-medium uppercase tracking-wide text-text-muted">
            Withdrawing {formatCurrency(amount)} of your {formatCurrency(balance, { compact: true })} balance · {reason.label.toLowerCase()}
          </p>
...
function InvestPanel({
  ...
  balance: number
  bracketNow: BracketKey
  onBracketNow: (k: BracketKey) => void
  bracketRetirement: BracketKey
  onBracketRetirement: (k: BracketKey) => void
  returnRate: number
  onReturnRate: (v: number) => void
  years: number
  onYears: (v: number) => void
}) {
  const conversionRate = bracketRate(bracketNow)
  const retirementRate = bracketRate(bracketRetirement)

  const path = useMemo(
    () => projectRothVsTraditionalPath(balance, conversionRate, retirementRate, returnRate, years),
    [balance, conversionRate, retirementRate, returnRate, years],
  )
  const conversion = useMemo(
    () => scenarioRothConversion(balance, conversionRate, returnRate, years),
    [balance, conversionRate, returnRate, years],
  )
  const last = path[path.length - 1]
  const delta = Math.abs(last.rothNet - last.traditionalNet)
  const isTie = delta < 1
  const rothWins = !isTie && last.rothNet > last.traditionalNet

  return (
    <div className="grid gap-8 lg:grid-cols-[280px_1fr]">
      <div className="space-y-6 rounded-3xl bg-surface-card p-6">
        <BracketPicker label="Tax bracket now (age 18)" value={bracketNow} onChange={onBracketNow} />
        <BracketPicker label="Tax bracket at retirement" value={bracketRetirement} onChange={onBracketRetirement} />
        <LabeledSlider
      ...
      <div className="rounded-3xl bg-surface-card p-6">
        <p className="text-xs font-medium uppercase tracking-wide text-text-muted">
          Spendable value at age {last.age}, starting from {formatCurrency(balance, { compact: true })} at 18
        </p>

        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <div className="rounded-2xl p-4" style={{ background: rothWins ? 'var(--surface-2)' : 'var(--status-good-bg)' }}>
            ...
            {!isTie && !rothWins && <p className="mt-1 text-xs font-semibold" style={{ color: 'var(--status-good)' }}>Wins by {formatCurrency(delta, { compact: true })}</p>}
          </div>
        ...
        {isTie && (
          <p className="mt-3 text-xs font-semibold text-text-secondary">
            About the same either way — the two tax rates cancel out.
          </p>
        )}
...
export function ScenarioPlanner() {
  ...
  const [bracket, setBracket] = useState<BracketKey>('mid')
  const [bracketNow, setBracketNow] = useState<BracketKey>('mid')
  const [bracketRetirement, setBracketRetirement] = useState<BracketKey>('mid')
  ...
  return (
    <Section
      ...
      description="At 18 the account becomes a traditional IRA. From there, the family has two basic choices — pull money out, or leave it invested."
      tone="muted"
    >
      <div className="mx-auto mb-8 max-w-lg rounded-3xl bg-surface-card p-6 text-center">
        <p className="text-sm font-medium text-text-primary">Account balance at 18</p>
        <p className="font-display mt-1 text-3xl text-text-primary">{formatCurrency(balance, { compact: true })}</p>
        <input
          type="range"
          min={10000}
          max={250000}
          ...
          className="mt-3 w-full accent-[var(--series-hero)]"
        />
        <p className="mt-1 text-xs text-text-muted">
          Based on the compounding calculator above — drag to try a different balance.
        </p>
      </div>
      ...
      {path === 'withdraw' ? (
        ...
        <InvestPanel
          ...
          onBracketNow={setBracketNow}
          bracketRetirement={bracketRetirement}
          onBracketRetirement={setBracketRetirement}
          returnRate={returnRate}
          onReturnRate={setReturnRate}
          years={yearsToRetirement}
          onYears={setYearsToRetirement}
        />
      )}
    </Section>
  )
